...
function truncateLastRow(rows: string[], width: number): string[] {
	...
	if (visibleWidth(last) >= width && width > 1) rows[rows.length - 1] = truncateToWidth(last, width - 1, "") + "›";
	return rows;
}
...
function updateAnsiState(current: string, sequence: string): string {
	...
	if (sequence === "\x1b[3m" || sequence === "\x1b[23m") return replaceAnsi(current, /\x1b\[(?:3|23)m/g, sequence === "\x1b[23m" ? "" : sequence);
	if (sequence === "\x1b[4m" || sequence === "\x1b[24m") return replaceAnsi(current, /\x1b\[(?:4|24)m/g, sequence === "\x1b[24m" ? "" : sequence);

...
Path icons default to portable unicode markers. Set `CODE_PREVIEW_PATH_ICONS=nerd` or `codePreview.pathIcons: "nerd"` if you use a Nerd Font, or `off` for plain path lists. Image rendering is left to pi's native image content renderer.
